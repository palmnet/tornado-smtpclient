...
#            self.file = self.sock.makefile('rb')
        while 1:
            try:
                line = yield self.stream.read_until(CRLF)
    ...
    def mail(self, sender, options=[]):
        ...
        # (code, msg) = yield self.docmd('mail', ('FROM:%s%s' % (smtplib.quoteaddr(sender), optionlist)).encode('ascii'))
        #raise Return((code, msg))
    ...
    def ehlo_or_helo_if_needed(self):
        ...
        if self.helo_resp is None and self.ehlo_resp is None:
            helo = yield self.ehlo()
            if not (200 <= helo[0] <= 299):
    ...
    def login(self, user, password):
        ...
        if authmethod == AUTH_CRAM_MD5:
            ...
            (code, resp) = yield self.docmd(encode_cram_md5(resp, user, password))
    ...
    def starttls(self, keyfile=None, certfile=None):
        ...
        (resp, reply) = yield self.docmd("STARTTLS")
        if resp == 220:
            if not _have_ssl:
